...
const Loan = (props) => {
    ...
    useEffect(() => {
        ...
        setLoad(true);
        axios.get(`${baseURL_PORTAL}finance1`, CONFIG).then(res => {
            let new_data = []; // difine a new object
  
            if (res.data.ReportsFinOne.length > 0) {  // looping  through the array for modifing the data
                for (let i = 0; i < res.data.ReportsFinOne.length; i++) {
                    let new_date = { new_date: moment(res.data.ReportsFinOne[i].created_on).format('DD-MM-YYYY') }
                    new_data.push(Object.assign(new_date, res.data.ReportsFinOne[i]));
                }
                ...
                return (
                    <Fragment>
                        ...
                        <button
                            ...
                            onClick={() => OpenSupportModal(record)}
                        >
                           Support
                        </button>
                    </Fragment>
  ...
  const handleChangeSuggestions = (e) => {
    ...
    const onSubmitHandleEdit = (e) => {
     ...
     // handle onsubmit for asking the support
     const onSubmitHandleEditSupport= (e) => {
        e.preventDefault();
        setLoad(true);
        let formatData = {
            id: state.report_id
        ...
        axios.put(`${baseURL_PORTAL}finance2/${edit_record.id}`, formatData, CONFIG).then(res => {
            setLoad(false);
            if (res.data.Success === true) {
                successToast("Sorted successfully");
                IsOpen(false);
    ...
    return (
        <div>
            ...
            >
                ...
                <br></br>
                <>
                    <Form className="form login-form" onSubmit={onSubmitHandleEditSupport} >
                            <div className="account__btns col-8 offset-2" >
                                <br />
                                <Button className="account__btn" type='submit' color="success"> {
                                    isLoading ? "Please wait..." : "yes!"
